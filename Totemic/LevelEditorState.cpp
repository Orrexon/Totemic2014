...
void LevelEditorState::entering()
{
	...
	AppendMenuA(hPowerupsDropDown, MF_STRING | MF_POPUP, ID_NEW_POWERUP, "New powerup");
...
bool LevelEditorState::update(float dt)
{
	...
	if (m_placingPowerup)
	{
		...
		{
			delete m_powerups[m_placingPowerupIndex];
			m_powerups[m_placingPowerupIndex] = nullptr;
...
void LevelEditorState::openFile()
{
	...
	if (!openpath.empty())
	{
		...
		m_powerups.clear();
		for (auto it = root["powerup"].begin(); it != root["powerup"].end(); ++it)
		{
...
void LevelEditorState::loadObject()
{
	...
	if (!objectFile.empty())
	{
		...
		loadObjectRaw(String::replace(path, "\\\\", "/"), sf::Vector2f(thor::random(0, 1920), thor::random(0, 1080)));
	}
	else
	{
