...
static int init_event_queue(void)
{
	...
	raw_hwnd = pCreateWindowExA(0, (LPCTSTR)class_name, (LPCTSTR)win_name, WS_OVERLAPPEDWINDOW,
...
static void cleanup_window(void)
{
    ...
    if (class_atom)
    {
        pUnregisterClassA((LPCTSTR)class_name, pGetModuleHandleA(NULL));
        class_atom = 0;
    } /* if */
} /* cleanup_window */
                            ...
                            SP_DEVINFO_DATA *data)
{
    ...
    while (1)
    {
        ...
        if (!pSetupDiGetDeviceInstanceIdA(devinfo, data, (PTSTR)buf, bufsize, NULL))
            continue;
...
static int check_for_disconnects(ManyMouseEvent *ev)
{
    ...
    if (++i >= (unsigned int)available_mice)  /* check first in case of redetect */
        i = 0;
